	...
	public class Stage_14 extends MovieClip
	{
		...
		private function dropIn (e:MouseEvent):void
		{
			if (e.currentTarget.name == allObjects.paper_mc.name)
			{
				...
				e.currentTarget.stopDrag();
				TweenLite.to(e.currentTarget, 0.5, { x: allObjects.drop_site_mc.x, y:allObjects.drop_site_mc.YposBox.y + e.currentTarget.height / 2 , scaleX:0.8, scaleY:0.8} );
				allObjects.drop_site_mc.YposBox.y = allObjects.drop_site_mc.YposBox.y + e.currentTarget.height + 30
				}
			else
			{
			e.currentTarget.removeEventListener(MouseEvent.MOUSE_DOWN, dragFunc);
			e.currentTarget.stopDrag();
			TweenLite.to(e.currentTarget, 0.5, { x: allObjects.drop_site_mc.x, y:allObjects.drop_site_mc.YposBox.y + e.currentTarget.height / 2 } );
			allObjects.drop_site_mc.YposBox.y = allObjects.drop_site_mc.YposBox.y + e.currentTarget.height + 30
			}
